...
(() => {
    ...
    function refreshSessions() {
        ...
        if (sessionsState.loading || refreshInFlight)
            return;
        refreshInFlight = true;
        fetchJson(`${API}/sessions?limit=50&offset=0`)
            .then((data) => {
            const list = $("session-list");
            (data.items ?? []).forEach((raw) => {
                const existing = sessionsState.rows.get(raw.sessionId);
     ...
     */
    function rowsWrap() {
        return $opt("detail-rows")?.closest(".table-wrap") ?? null;
    }
    ...
    function renderStatus(st) {
        ...
        bar.appendChild(chip("마지막 자막 전송 " +
    ...
    function markStatusStale() {
        if (statusStale)
            return; // 연속 실패 — 칩은 하나면 된다
        statusStale = true;
        const bar = $opt("status-bar");
        if (!bar)
            return;
        // 칩이 하나도 없으면(최초 로드부터 실패) 덧붙일 옛 값 자체가 없다.
        bar.appendChild(chip(bar.children.length > 0 ? "조회 실패 — 앞의 값은 마지막 성공분" : "상태 조회 실패", "bad"));
    }
    function pollStatus() {
        if (statusInFlight)
            return;
