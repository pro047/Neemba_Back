...
| **WU-F** | **미착수** (2026-08-02 신설) | 상태 수명주기 정합성 3건. §11 참조 |
...
**수정 방향(미확정 — 코드 확인 후 결정)**: ① 세션 teardown 에서 pending blip 을
닫기(세션 종료 사유로 마감) ② blip 기록 시 현재 session_id 대조 ③ startup
`close_stale_sessions` 에 blip 정리 추가. **①이 WU-D 와 같은 자리라 유력하다.**
...
- 세션을 paused/순단 상태로 종료해도 `ws_blips` 에 열린 행이 남지 않는다
- 한 세션에서 순단이 여러 번 나면 행도 여러 개 생긴다
- publisher 인스턴스가 교체돼도 드롭 카운터가 뒤로 가지 않는다
- prod 잔존 고아 행(현재 id 3) 정리 방법 결정 — 마이그레이션 or 수동 UPDATE
