...
| WU-D | **구현 완료·머지 대기** (2026-07-30) | PR **#63** `feature/gauge-teardown-paths` (`9956e16`). 2026-08-02 기준 develop 미머지 — 이전 판의 "미착수" 는 갱신 누락이었다 |
| WU-E | 미착수 | 통합 리뷰는 별도 세션 |
| **WU-F** | **구현·테스트 완료** (2026-08-03) | F-1(python)·F-2(node) 각각 브랜치 분리, release 는 하나. F-3 은 WU-D 에 포함. §11 참조 |
...
- 2026-08-02 (주일예배 prod 모니터링 세션): 예배 중 5분 간격 실시간 감시.
  서버측 장애 0건. 관측 3건이 전부 같은 패턴("상태가 인스턴스·세션 경계를
  넘으면 어긋난다")으로 수렴해 **WU-F 신설**(§11). WU-D 가 이미 구현돼
  PR #63 으로 열려 있는데 §7 이 "미착수" 로 남아 있던 것을 정정. 코드 변경
  없음(문서만). 다음 세션: WU-E 통합 리뷰 → release.
- 2026-08-03 (WU-F 세션): F-1 근인을 코드 대조로 확정(가설 단계 없이 prod 3행이
  그대로 재현됨) → 구현 → **수정 전 코드에서 새 테스트 5건 실패**를 먼저 확인한
  뒤 통과로 뒤집어 회귀 가드 성립을 실증. F-2 도 폐기 스크래치 테스트로 "드롭
  2건이 1로 보고됨" 을 재현한 뒤 카운터로 교체. 완료 기준의 "열린 행이 남지
  않는다" 를 §4-7 결정 2 와 맞게 정정. 다음: 커밋·PR → release.

---

## 11. WU-F — 상태 수명주기 정합성 (2026-08-02 신설)

**한 줄**: 성공 경로에서 세운 상태를 종료·교체 경로에서 안 내리는 결함이
게이지 하나(WU-D)에 그치지 않고 **DB 레코드와 카운터에도 같은 형태로 있다.**

WU-D 와 같은 릴리스에 묶을 것. 셋 다 원인 구조가 같아 리뷰를 한 번에 하는 편이
싸고, 개별로 흩으면 "또 그 버그" 가 반복 발견된다.

### F-1. `ws_blips` pending 행이 세션 종료 시 안 닫힌다 **(1순위)**

순단이 시작되면 `ws_blips` 에 `reconnected_at IS NULL` 행이 생기는데, 재연결 없이
세션이 끝나면 그 행이 **열린 채 남는다.** 그리고 다음 세션의 첫 재연결이 그
행을 닫아버린다.
...
id | session_id  | disconnected_at        | reconnected_at         | duration_ms
 2 | 2c2d72c8... | 2026-07-29 11:21:29+00 | 2026-08-02 02:02:18+00 |  312048333
 3 | 2d3aa2ab... | 2026-08-02 02:13:20+00 |        (NULL)          |
```

- 행 2: 7/29 세션의 미종료 행을 **8/2 새 세션의 재연결이 닫았다.** duration 3.6일,
  session_id 도 오늘 세션(`2d3aa2ab`)이 아니라 7/29 세션이다.
- 행 3: 오늘 또 같은 방식으로 열린 채 남았다. 다음 예배가 이걸 닫으면 반복된다.

**더 나쁜 점 — 기록 누락**: 오늘 순단은 최소 3회(02:02·02:13·02:27)인데 행은
**1개만** 생겼다. pending 슬롯이 하나뿐이라, 안 닫힌 상태에서는 이후 순단이
아예 기록되지 않는 것으로 보인다.

**영향**: `ws_blips` 는 handover §4-7 이 "순단 원인 판별" 을 위해 만든 테이블이고,
가설(① 기기 Wi-Fi ② 절전 ③ NAT)을 **빈도와 지속시간으로** 가리는 것이 목적이다.
가짜 3.6일 행 하나와 누락된 2건이 그 판단을 통째로 무너뜨린다.

**근인 — 2026-08-03 코드 대조로 확정(가설 아님)**: `detach()` 가 blip 슬롯을
반납하지 않는다. 관련 상태는 `_blip_start_task` 하나뿐인데,

- `websocket.py` `_open_blip_locked`: 열린 blip 이 있으면 **새 순단을 기록하지 않는다**
- attach 의 슬롯 리셋 분기는 `self._session_id is not None` 일 때만 돈다
- `detach()` 는 `_session_id = None` 으로 만들면서 태스크는 남긴다

→ detach 이후 attach 는 리셋 조건을 **영원히** 만족하지 못한다. 그래서 슬롯이
고착돼 이후 순단이 전부 무시되고, 다음 세션의 재접속이 `same_session` 판정을
통과하는 순간 **남의 행**을 닫는다.

prod 3행이 이 경로로 한 칸씩 재현된다: 7/29 순단 insert(행 2) → 7/30 수동
stop(D3)이 detach → 8/2 02:02:17 순단은 가드에 막혀 **행 없음** → 02:02:18
재접속이 행 2 를 flushed=4·3.6일로 마감 → 02:13:20 순단 insert(행 3) →
02:13:27 stop 후 세션 5회 교체 → 02:27:36 순단 **행 없음**.

**수정**: ① `detach()`·세션 교체에서 슬롯 반납(`_release_blip_locked`)
② `_blip_session_id` 로 blip 주인을 따로 들고, 종결 판정을
`_reconnect_waiting and same_session` → **주인 일치**로 교체. ②는 keepalive 가
300초 뒤 대기를 포기한 다음 온 재접속도 마감되게 만든다. ③(startup 정리)은
근인을 안 건드려 매 예배 재발하므로 채택하지 않는다.

### F-2. `neemba_publish_buffer_dropped_total` 이 인스턴스 교체 시 0 으로 덮인다

이름은 `_total`(카운터)인데 실제로는 **인스턴스별 total 을 set 하는 게이지**다.
publisher 인스턴스가 새로 생기면 0 으로 리셋된다.

**2026-08-02 prod 실측** — 로그에는 드롭이 찍혔는데 메트릭은 0:

```
02:13:48  publish buffer: span dropped after stop, total dropped=1
02:14:36  publish buffer: span dropped after stop, total dropped=1
02:14:37  publish buffer: span dropped after stop, total dropped=2
02:16:13  publish buffer: span dropped after stop, total dropped=1
→ neemba_publish_buffer_dropped_total = 0
```

`total dropped` 가 1→1→2→1 로 **되돌아가는 것** 자체가 인스턴스별 카운터라는 증거다.
자막 유실 5건이 메트릭상 0건으로 보고됐다.

**영향**: 유실 감지가 메트릭으로 안 된다. monitor 사이드카가 이 값을 본다면
유실 경보가 영영 안 울린다.

### F-3. (WU-D 로 이미 수정됨 — 배포만 남음)

`neemba_stt_paused` 가 종료 경로에서 안 내려가는 건. PR #63 에 포함.
2026-08-02 예배 내내 게이지가 1 로 고정돼 monitor 가 오탐 경보를 반복했다
(prod 미배포 상태라 관측된 것).

### 완료 기준 (2026-08-03 정정)

이전 판의 "열린 행이 남지 않는다" 는 §4-7 결정 2(`reconnected_at NULL 잔존 =
미복귀`)와 충돌해서 정정했다. 미복귀 종료의 올바른 표현은 **행이 없는 것이
아니라 NULL 로 남은 행**이고, 모니터도 이미 그 규약으로 `미복귀` 뱃지를
그린다(`src/app.ts`). 따라서 이 작업에 마이그레이션도 sweeper 도 없다.

- 열린 행은 **그 세션 것 하나뿐**이고, 다른 세션이 그 행을 닫지 않는다
  — **P1(#81) 이후 이 항목은 세션 단위로만 참이다.** 같은 세션 안에서는 소켓별로
  행이 여러 개 열릴 수 있고, `_match_open_blip_locked` 가 세션 FIFO 로 매칭하므로
  **A 가 끊긴 뒤 처음 붙는 새 청취자 C 가 A 의 행을 "복귀" 로 닫는다.**
  다중 청취자 세션에서 `reconnected_at NULL = 미복귀` 와 모니터의 `미복귀` 뱃지는
  근사치다 (`multi-listener-p1-plan.md` §2 "blip 해석 주의")
- 세션이 미복귀로 끝나면 `reconnected_at` 은 NULL 로 두되 다시는 방류되지 않을
  pending 을 `lost_count` 로 마감한다 (행이 피해량을 갖는다)
- 한 세션에서 순단이 여러 번 나면 행도 여러 개 생긴다
- publisher 인스턴스가 교체돼도 드롭 카운터가 뒤로 가지 않는다
- prod 잔존 행 정리: **행 3 은 정상이라 건드리지 않는다**(실제로 미복귀 종료).
  틀린 것은 **행 2** 뿐이므로 1행 수동 UPDATE 로 되돌린다 — 마이그레이션 불필요

  ```sql
  UPDATE app.ws_blips
     SET reconnected_at = NULL, duration_ms = NULL, flushed_count = NULL
   WHERE id = 2;
  ```

  8/2 02:02·02:27 두 순단은 insert 자체가 없었으므로 **복구 불가**다.
