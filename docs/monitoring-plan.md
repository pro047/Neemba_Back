...
| 3 | Alembic + 스키마 | 검증완료(런타임) | **A. 정적**: `pyproject.toml`에 `alembic`+`psycopg[binary]` 추가→`uv lock` 재생성→`uv lock --check` **exit 0**(`alembic 1.18.4 / psycopg 3.3.4 / sqlalchemy 2.0.50` 추가됨). `python -m py_compile main.py src/config.py src/database/pool.py migrations/env.py migrations/versions/0001_initial_monitor_schema.py`→**exit 0**. **B. 런타임 마이그레이션 스모크(핵심)**: dockerd 기동→`mirror.gcr.io/library/postgres:16-alpine` 우회 pull·태깅→더미 `.env.prod`→`docker compose -f docker-compose.prod.yml --env-file .env.prod up -d postgres`→**healthy(t=8s)**. 호스트에 `alembic psycopg[binary] python-dotenv` 설치 후 컨테이너 브리지 IP(`172.18.0.2`)로 `POSTGRES_HOST`만 오버라이드→**`alembic upgrade head` → `Running upgrade -> 0001_initial` exit 0**. 테이블 검증: `\dt app.*`→`app.sessions`+`app.translations`, `SELECT to_regclass('app.translations'),to_regclass('app.sessions')`→**둘 다 non-null**, 인덱스 `ix_translations_session_id`+`ix_translations_created_at` 생성 확인. **down/up 왕복**: `alembic downgrade base`(테이블·`app`스키마 제거, `app`스키마 count=0)→`alembic upgrade head`(테이블 재생성 non-null) 모두 exit 0. **오프라인**(`upgrade head --sql`)도 `CREATE SCHEMA/TABLE/INDEX app.*` SQL 정상 생성. **alembic_version은 `public`에 위치**(downgrade의 `DROP SCHEMA app CASCADE`와 충돌 회피). 정리: 스모크 후 `compose down -v`+더미 `.env.prod` rm. **[독립검증 2026-06-02 — 합격(런타임 재확인)]** 무수정 검증으로 전부 재현: 정적(`uv lock --check` exit 0 / `uv export`에 `alembic 1.18.4`·`psycopg 3.3.4`·`sqlalchemy 2.0.50`·런타임 `asyncpg 0.30.0` 유지 / 5파일 `py_compile` exit 0 / `env.py`가 오프라인·온라인 양쪽 `set_main_option("sqlalchemy.url", get_postgres_sync_url())` 코드주입·`alembic.ini`엔 `sqlalchemy.url` 비밀 하드코딩 없음) → 런타임(dockerd→mirror postgres healthy t=3s, 브리지IP `172.18.0.2` 직결, **`alembic upgrade head` → `Running upgrade -> 0001_initial` exit 0**, `to_regclass` 둘 다 non-null, 인덱스 2개+pkey, **public.alembic_version non-null / app.alembic_version NULL**, version=`0001_initial`) → **down/up 왕복**(`downgrade base` exit 0: app테이블 count=0·둘 다 NULL·public.alembic_version 테이블 보존(rows=0) → `upgrade head` exit 0: 테이블 재생성 non-null·인덱스 2개 복구) → 오프라인 `--sql` DDL 정상 → 엔트리포인트 배선(`entrypoint.sh` `alembic upgrade head`→`exec "$@"`, Dockerfile runtime `ENTRYPOINT`+dev uvicorn/prod gunicorn `CMD` 인자보존) 코드 확인 → Phase2 회귀(`grep services.python.src.config` 0건·`get_postgres_config` 5키·pool/main `py_compile` OK). 버그 0건. | `claude/monitor-phase3-alembic-schema-0fRe8` (독립검증: `claude/monitor-phase3-validation-vOh3Z`) | sync 드라이버 `psycopg[binary]`(런타임은 asyncpg 유지). DATABASE_URL=POSTGRES_*에서 조립(`get_postgres_sync_url()`, 신규 시크릿 無). 파티셔닝은 Phase 7로 이월(단순 인덱스 테이블). 엔트리포인트=`entrypoint.sh`(`alembic upgrade head`→`exec "$@"`). ⚠️호스트 alembic은 PyPI 도달 OK(컨테이너 내부 pip는 불가)→호스트 직결로 검증 |
...
- (2026-06-02, **Phase 3 독립 검증 세션**, 브랜치 `claude/monitor-phase3-validation-vOh3Z` — alembic-schema `0fRe8`를 fast-forward로 끌어와 무수정 검증) **판정: 합격(검증완료·런타임 재확인). 발견 이슈 0건, 코드/스키마/문서 무수정(메모만 갱신).** 정적: `uv lock --check` exit 0 / `uv export`에 `alembic 1.18.4`·`psycopg 3.3.4`·`sqlalchemy 2.0.50`·런타임 `asyncpg 0.30.0` 유지 / `py_compile`(main·config·pool·env·0001) exit 0 / `env.py`가 오프라인·온라인 양쪽 `set_main_option("sqlalchemy.url", get_postgres_sync_url())`로 코드주입·`alembic.ini` `sqlalchemy.url` 미설정(비밀 하드코딩 없음)·`get_postgres_sync_url()`는 POSTGRES_*에서 `postgresql+psycopg://` 조립(신규 DATABASE_URL 시크릿 없음). 런타임(호스트 alembic→컨테이너 브리지 IP `172.18.0.2` 직결): `alembic upgrade head` → `Running upgrade -> 0001_initial` exit 0 → `to_regclass('app.translations'/'app.sessions')` 둘 다 non-null·인덱스 `ix_translations_session_id`/`ix_translations_created_at` 존재·`public.alembic_version` non-null·`app.alembic_version` NULL(설계대로) → **down/up 왕복**: `downgrade base` exit 0(app테이블 0개·둘 다 NULL·`public.alembic_version` 테이블 보존 rows=0) / `upgrade head` exit 0(테이블·인덱스 재생성) → 오프라인 `--sql`도 `CREATE SCHEMA/TABLE/INDEX app.*`+`alembic_version`(public) 정상. 엔트리포인트: `entrypoint.sh`=`alembic upgrade head`→`exec "$@"`, Dockerfile runtime `ENTRYPOINT`+dev uvicorn/prod gunicorn `CMD` 인자보존 확인. ⚠️full app boot은 NATS/DeepL/WS require_env로 의도상 미시도(alembic 단독만 검증). Phase 2 회귀: `grep services.python.src.config` 0건·`get_postgres_config` 5키 유지·pool/main `py_compile` OK. 정리: 컨테이너 `down -v`·더미 `.env.prod` rm. **다음(Phase 4) 주의**: 공용 마스킹 유틸 + `pusher.py` 경로 원문+번역문 마스킹 후 비동기 INSERT(`app.translations`) + 모니터 전용 WS(`/ws/monitor?sessionId=`) + `/internal/sessions/stop` 멱등화(+`ended_at`/`translation_count`).
