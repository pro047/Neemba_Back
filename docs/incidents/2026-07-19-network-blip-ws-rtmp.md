...
- **결함 1의 송출 주체는 mvp 앱이 아니라 OBS + obs-multi-rtmp 플러그인(sorayuki)** 이었다.
  플러그인에 기초 자동 재연결("Reconnecting in 2.00s")은 있으나, 특정 연결 에러 후
  재연결이 먹통이 되는 미해결 버그가 있다(sorayuki/obs-multi-rtmp#470, 2025-02 보고·open:
  "재연결 버튼 무반응, OBS 재시작으로만 복구") — 이번 장애의 "앱 완전 재시작으로만
  복구"(타임라인 5)와 정확히 일치. 대응 옵션은 인수인계 §4-4-1 참조.

## 운영 참고

- **RTMP 송출 성공 판정 기준 = node 로그의 `rtmp on_publish` 라인** (publish 연결 시도마다 찍힘, 유지 중엔 안 찍힘).
- 순단 후 송출이 안 살아나면: obs-multi-rtmp 스트림 토글 → 안 되면 OBS 완전 재시작 (플러그인 버그 #470).
- `Stt giving up`까지 간 세션은 오디오가 살아나도 복구 안 됨 → on_publish 확인 후 스탑→스타트로 새 세션. (§4-4-2 수정 배포 후에는 `Stt paused`→오디오 재유입 시 자동 재개로 완화됨.)
