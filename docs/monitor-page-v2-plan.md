...
## 8. 리뷰 잔여 — 모니터 페이지 안정성 3건 (2026-08-06 조사, 구현은 다음 세션)

§7 이 남긴 "리뷰 잔여 8건" 중 **안정성 3건만** 현 코드와 대조했다. 셋 다 남아 있고,
①은 헤드리스로 재현까지 했다. **줄번호는 2026-08-06 기준**(수정이 들어가면 밀린다).
...
`monitor-page-v3-plan.md` §8 의 하니스와 같은 것에 스위치 하나를 더한 것이다 —
서버가 `index.html` 을 내려줄 때 `#detail-new` 버튼을 지워 캐시 스큐를 만든다.
세션 scratchpad 에만 있고 리포에는 없다. 재작성이 필요하면 v3 §8 설명 + 위 한 줄로 충분하다.
