...
| 통합 리뷰 | **완료 · prod 배포** | 2026-08-02 | 전 범위 재검증(주장 전부 재현) + 확정 3건 + 코드 리뷰 7건 반영, pytest 146. PR #61 머지 |
| 정적 자산 수정 | **완료 · prod 배포** | 2026-08-02 | 슬래시 없는 `/monitor` 먹통(Phase 6 이래) + 캐시로 배포 미반영. PR #67·release #68 |

**이 계획(monitor-page-v2)은 2026-08-02 로 종료됐다.** WU1~WU5 전부 prod(`4dfe434`)에
있다. 후속 작업은 `docs/monitor-page-v3-plan.md` 로 넘어간다.

배포 이력: release #57(WU1) → #65(WU2·3·4 + 경보 수정) → #66(WU5 + 통합 리뷰) → #68(정적 자산).
...
- 2026-08-02 (정적 자산 서빙 수정): WU5 배포 후 prod 화면에서 드러난 nginx 결함 2건.
  ...
  **release #68 배포 완료**(`4dfe434`). prod 실측: `GET /monitor` → `301`
  `Location: /monitor/`(상대 경로), 따라가면 `/monitor/`에서 `401`(인증 단계 도달),
  `Cache-Control: no-cache`. **이 배포 직후 한 번은 하드 리프레시가 필요하다** —
  기존에 캐시된 index.html 에는 no-cache 가 없어 그 캐시가 비워져야 새 헤더가 먹는다.
  그 이후 배포부터는 자동 갱신된다.
  **여기서 monitor-page-v2 는 종료.** 다음에 뭘 할지는
  `docs/handover-2026-08-02.md` §2 우선순위 큐를 볼 것 — 이 세션 끝에 재정렬했다.
  1순위는 WU-F(`session-lifecycle-plan.md` §11), 2순위가 V3-A 다.

## 8-0. 리뷰 잔여 8건 — 현재 상태 (2026-08-09 갱신)

§7 의 2026-07-29 항목이 잔여 8건을 한 문장으로 뭉쳐 적어 두어 무엇이 닫혔는지
읽히지 않는다. **그 문장은 날짜 박힌 기록이라 고치지 않고, 대조표를 여기 둔다.**

| # | 항목 | 상태 |
|---|---|---|
| ① | 순단 탭 새로고침 실패 시 `nextOffset` 미초기화 | **완료** 2026-08-09 (`fix/monitor-blips-pagination`) |
| ② | OFFSET 페이지네이션 중복 행 | **완료** 2026-08-09 (같은 브랜치, 키셋 전환) |
| ③ | `activeSessions` 가 버려진 세션 포함 | **남음** |
| ④ | 10s 폴링 in-flight 가드 | **완료** release #80. teardown 부재는 §8-② 에서 결함 아님으로 기각 |
| ⑤ | 캐시 스큐 시 init 예외로 페이지 백지화 | **완료** release #80 (`$opt`/`wire`) |
| ⑥ | `nodeUp` 이 임의 2xx 를 healthy 로 판정 | **남음** |
| ⑦ | 폴링 1회 실패에 칩 전체 소거 | **완료** release #80 (`markStatusStale` 이 덧붙인다) |
| ⑧ | 스크레이프 실패 무로깅 | **남음** |

**남은 3건의 위치** (2026-08-09 코드 직독):

- ③ `monitor_query_repository.count_active_sessions` 가 `ended_at IS NULL` 로만
  센다. P1 D4 로 종료 경로가 `on_publish_done` + 유예 하나뿐이라, 훅이 한 번
  유실되면 그 행이 영영 "활성" 으로 남아 칩이 거짓을 말한다. **셋 중 이것만
  표시 열화가 아니라 상태 자체의 거짓이다** — ①② 를 우선한 근거가 그대로 적용된다.
- ⑥ `node_metrics.fetch_node_gauges` 가 `raise_for_status()` 로 비 2xx 만 거른다.
  200 인데 게이지가 하나도 없으면 `parse_gauges` 가 빈 dict 를 주고
  `main.py` 의 `nodeUp = gauges is not None` 이 True 가 된다 — 칩은 초록인데
  하위 값 3개가 전부 null.
- ⑧ 같은 함수의 `except Exception: return None` 이 원인을 통째로 삼킨다.
  같은 세션에 손본 DB 경로(`monitor_status` 의 `logger.exception`)와 어긋난다.

이 8건과 **별개로** `handover-2026-08-02.md` §10 의 "모니터 폴러 사각지대 2건"
(플래그 고착 3곳, `fetchJson` 타임아웃 부재)이 우선순위 큐 4번에 따로 있다.
2026-08-09 코드 리뷰가 지적한 **`loadBlips` 의 in-flight 가드가 reset 을 통째로
삼키는 문제**(요청이 떠 있는 동안 누른 [새로고침]이 조용히 무시된다)도 성격이
같아 그쪽에 얹는 것이 맞다. ① 이 막으려던 오판이 다른 경로로 되살아나므로
남은 3건보다 우선순위가 높다.

## 8. 리뷰 잔여 — 모니터 페이지 안정성 3건 (2026-08-06 조사 → 2026-08-08 구현 완료)

> **완료됨 (2026-08-08, release #80 / PR #79).** 아래 3건은 전부 고쳐져 prod 에
> 배포됐고 헤드리스로 검증했다(9/9, 음성 대조군 6건 실패 확인).
> **구현 결과·코드리뷰 반영분·남은 사각지대는 `handover-2026-08-02.md` §10 에 있다.**
> 아래 본문은 **조사 시점(2026-08-06)의 기록**으로 남긴다 — 왜 그렇게 고쳤는지의
> 근거이지 할 일 목록이 아니다. **줄번호는 2026-08-06 기준이라 현 코드와 다르다.**

§7 이 남긴 "리뷰 잔여 8건" 중 **안정성 3건만** 현 코드와 대조했다. 셋 다 남아 있고,
①은 헤드리스로 재현까지 했다.

**왜 지금 골라냈나**: 8/9 주일예배는 WU-F F-1·F-2 와 V3-A 를 한꺼번에 검증하는
자리다. 그날 모니터 페이지가 죽으면 **검증 기회 3건을 통째로 잃고 다음 예배까지
밀린다.** 평소라면 표시 열화지만 그날만은 관측 도구의 신뢰도가 결과를 좌우한다.

### ① 캐시 스큐 → 초기화 중단 (가장 심각)

`$()` 는 요소가 없으면 throw 한다(`src/app.ts:180`). 전체가 IIFE 한 덩어리(`:156-1220`)
라 **던진 지점 이후의 배선이 통째로 안 일어난다.** 초기 데이터 로드가 IIFE 끝쪽이라
목록도 상태도 비는 결과가 된다.

**V3-A 가 노출을 넓혔다.** `$("detail-new").addEventListener(...)`(`:723`)가 이번
릴리스에 새로 생긴 요소를 가리키므로, release #73 이전 `index.html` 을 캐시한
브라우저가 새 `app.js` 를 받으면 정확히 이 예외를 맞는다.

**재현**(옛 index.html + 새 app.js, 헤드리스 Chrome):

```
초기화 중 던진 예외: Error: missing element: #detail-new
세션 목록 행 수      : 0     (정상 2)
상태 칩 개수         : 0     (정상 5)
세션 상태 문구       : ""    (정상 "2개 세션")
```

탭 전환만 살아남는데(`true`), 탭 배선이 예외 지점보다 **위**에 있어서지 설계가
견딘 게 아니다 — 우연이다.

**수정 방향**: 초기화 배선을 요소가 없으면 건너뛰게 한다(`wire(id, ev, fn)` 류 헬퍼가
없으면 `console.warn` 후 무시). "죽은 페이지" 를 "죽은 버튼 하나" 로 바꾸는 것이 목표다.
`$()` 를 쓰는 초기화 지점을 훑어야 해서 셋 중 범위가 가장 크다.

### ② 폴링 in-flight 가드 — 절반만 있다

| 타이머 | 주기 | 가드 |
|--------|------|------|
| `pollStatus`(`:1044`, `setInterval :1054`) | 10초(`STATUS_POLL_MS :991`) | **없음** |
| `refreshSessions`(`:471`, `setInterval :508`) | 30초(`SESSIONS_POLL_MS :469`) | `sessionsState.loading` 을 **읽기만** 하고 세팅하지 않는다(`:473`). 세팅은 전부 `loadSessions`(`:405·410·452`) |

`pollStatus` 가 §7 이 말한 "10s 폴링" 이다. `/status` 가 10초 넘게 걸리면 — 즉 DB 가
느려 **정확히 상태를 봐야 할 때** — 요청이 쌓이고 응답마다 칩이 다시 그려져 깜빡인다.
`refreshSessions` 는 사용자 조회와 겹치는 것만 막고 자기들끼리는 못 막는다(30초라 여유는 있다).

**수정 방향**: `pollStatus` 에 in-flight 플래그, `refreshSessions` 는 자기 플래그를 갖게.

**teardown 부재는 결함으로 보지 않는다(2026-08-06 판단).** SPA 가 언마운트되지 않아
정리할 시점 자체가 없다. §7 의 8건 중 이 항목만 실질이 없다.
...
.catch(() => {            // :1047
  ...
  clear(bar);             // 5개 칩 전부 날림
  bar.appendChild(chip("상태 조회 실패", "bad"));
});
```

10초 주기라 **한 번의 일시적 실패로 모든 상태 정보가 사라진다.** 바로 위
`refreshSessions` 는 같은 상황에서 "폴링 실패는 조용히 넘긴다 — 다음 틱에 재시도"
라고 주석까지 달고 아무것도 지우지 않는다. **같은 파일 안에서 두 폴링의 실패 철학이 반대다.**

**수정 방향 (2026-08-06 사용자 결정)**: 기존 칩을 **유지**하고 "조회 실패" 표시만
덧붙인다. 값을 지우는 쪽과 비교해 오래된 값을 보여줄 위험이 있지만, 실패했다는 사실을
같이 표시하므로 사용자가 그 값을 최신으로 오인하지 않는다. 아무것도 안 보이는 것보다
"이 값은 N초 전 것" 이 낫다는 판단.

### 재현 하니스 (2026-08-08 확장)

`monitor-page-v3-plan.md` §8 의 하니스에 스위치를 더한 것이다. 세션 scratchpad 에만
있고 리포에는 없다 — 재작성이 필요하면 v3 §8 설명 + 아래로 충분하다.

- `SKEW=a,b` — `index.html` 을 내려줄 때 그 id 를 가진 줄을 지워 캐시 스큐를 만든다
- `REVERT=<name>` — `app.js` 를 내려줄 때 특정 수정을 되돌린다 (음성 대조군)
- `STATUS_FAIL=2,3` / `STATUS_DELAY=ms` / `SESSIONS_HANG_AFTER=n` — 폴링 실패·지연·무응답
- `/_ctl/wsdrop` — 열린 이벤트 소켓을 끊어 재연결 여부를 본다. `/_ctl/stats` 로 카운터 조회

**하니스를 다시 쓸 사람이 반드시 알아야 할 두 가지** (2026-08-08 에 각각 한 번씩 틀렸다):

1. **폴링 in-flight 가드를 "서버가 받은 요청 수" 로 측정하면 안 된다.** Chrome 은 같은
   URL 의 미완료 GET 뒤로 후속 동일 요청을 캐시 엔트리 락에 세운다 — 페이지가 6번
   `fetch` 해도 서버는 0건을 받는다. 가드 있는 빌드와 없는 빌드가 똑같이 나와 대조군이
   무의미해진다. CDP `Page.addScriptToEvaluateOnNewDocument` 로 app.js 보다 먼저
   `window.fetch` 를 감싸 **페이지가 시도한 횟수**를 세라.
2. **WS 업그레이드를 거절하면 안 된다.** 페이지가 재연결 배너로 `#sessions-status` 를
   덮어써 무관한 단정이 깨진다. 최소 RFC6455 핸드셰이크
   (`sha1(key + 258EAFA5-E914-47DA-95CA-C5AB0DC85B11)` → base64)만 응답하고 프레임은
   안 보내면 된다.
